...
void parse_csv(std::string filename, std::vector<Course>& courses) {
    ...
    while (std::getline(fin, info)) {   
        ...
        s_info = split(info, ',');
        Course course(s_info[0], s_info[1], s_info[2]);
...
void write_courses_offered(std::vector<Course>& all_courses) {
    /* (STUDENT TODO) Your code goes here... */
    std::ofstream fout("student_output/courses_offered.csv", std::ios::out);

    fout << "Title,Number of Units,Quarter\n";
    auto course = all_courses.begin();

    while (course != all_courses.end()) {
        ...
        if (quarter != "null") {
            ...
            course = all_courses.erase(course);
        }
        else {
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
    /* (STUDENT TODO) Your code goes here... */
    std::ofstream fout("student_output/courses_not_offered.csv", std::ios::out);
